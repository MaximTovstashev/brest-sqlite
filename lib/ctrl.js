...
class Ctrl {
    ...
    init (brest, callback) {
        ...
        const controllersPath = path.join(path.dirname(require.main.filename), brest.getSetting('sqlite.controllers') || DEFAULT_CONTROLLERS_PATH);
        ...
        async.waterfall([
             ...
             */
            (next_step) => {
                if (!_.isEmpty(brest.getSetting('sqlite.customControllers'))) {
                  this.customControllers = brest.getSetting('sqlite.customControllers');
                }
                    ...
                    async.waterfall([
                      ...
                      (controller_next_step) => {
                            if (this.customControllers) {
                                if (_.isObject(this.customControllers)) {
                                  const controller = this.customControllers[table_name] ? new this.customControllers[table_name](table) : new Controller(table);
                                  return controller_next_step(null, controller);
                                }
