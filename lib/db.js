...
class DB {
    ...
    connect(brest, settings, callback) {
        ...
        async.waterfall([
             ...
             */
            next => {
                if (!_.isEmpty(this.settings.customModels)) {
                      ...
                      table_next => {
                          if (this.customModels) {
                              if (this.settings.customModels) {
                                  if (this.settings.customModels[table_name]) {
                                    return table_next(null, this.settings.customModels[table_name]);
                                  }
                                  return table_next(null, Table);
                              }
    ...
    query(query, params = []) {
        ...
        return new Promise((resolve, reject) => {
            this.db.all(formatted_query, (err, rows) => {
              ...
              if (this.settings.log) console.log(rows);
              resolve(rows);
            });
          })
    }
